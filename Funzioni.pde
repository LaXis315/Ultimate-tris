void mousePressed(){
  ...
  float x = map(mouseX,0,width,0,3);
  float y = map(mouseY,0,height,0,3);
...
void controllotris(Tris trisc,int pos){ //controlla il trisc(trisc) per vedere se é stato vinto
  ...
  if(tris[pos].attivo == 2 || tris[pos].attivo == 3 || tris[pos].attivo == 4)
    controlloultimate();
}
...
void colorifondo(int i){
    ...
    if(tris[i].attivo == 2)
      ...
      stroke(#E51523); //rosso
    else if(tris[i].attivo == 3)
      stroke(#791A00); //nero
      
    if(tris [i].attivo == 1 && stato == 1)
      fill(#FBFF46); //giallo
    else if(tris [i].attivo == 2)
      fill(#76D827);
    else if(tris [i].attivo == -2)
